...
class Candle(object):
    ...
    def logic(self, row_idx):
        raise Exception(
            "Implement the logic of {}".format(self.get_class_name)
        )

    def has_pattern(self, candles_df, ohlc, is_reversed):
        self.prepare_data(candles_df, ohlc)
        ...
        if self.is_data_prepared:
            ...
            candles_df = candles_df.join(
                pd.DataFrame(results, 
                    columns=["row", self.target]).set_index("row"),
                    how="outer")
        ...
        if isinstance(candles_df, pd.DataFrame):
            if len(candles_df) >= self.required_count:
                if ohlc and len(ohlc) == 4:
                    ...
                    raise Exception(
                        "Provide list of four elements indicating columns in strings. "
                ...
                if not is_numeric_dtype(self.data[self.high_column]):
                    self.data[self.high_column] == pd.to_numeric(
                        self.data[self.high_column])

                self.is_data_prepared = True
            else:
                raise Exception(
                    "{} requires at least {} data".format(self.name, self.required_count))  
